...
namespace BookStore.DAL.Repositories
{
    public class CartItemRepository : IRepository<CartItem>
    {
        ...
        public CartItemRepository (ILogger<CartItem> logger, BOOK_STOREContext appDbContext)
        {
            _logger = logger;
            _appDbContext = appDbContext;
        }
        public async Task<CartItem> Create(CartItem model)
        {
            try
            {
                if (model != null)
                {
                    var obj = _appDbContext.Add<CartItem>(model);
        ...
        public IEnumerable<CartItem> GetAll()
        {
            try
            {
                var obj = _appDbContext.CartItems.ToList();
        ...
        public CartItem GetById(string? Id)
        {
            try
            {
                if (Id != null)
                {
                    var Obj = _appDbContext.CartItems.FirstOrDefault(x => x.Id == Id);
                    if (Obj != null) return Obj;
                    return null;
                }
                    return null;
        ...
        public Task<CartItem> GetByIdAsync(string id)
        {

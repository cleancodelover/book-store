...
namespace BookStore.DAL.Repositories
{
    public class ReviewRepository : IRepository<Review>
    {
        ...
        public ReviewRepository (ILogger<Review> logger, BOOK_STOREContext appDbContext)
        {
            _logger = logger;
        ...
        public async Task<Review> Create(Review model)
        {
            try
            {
                if (model != null)
                {
                    var obj = _appDbContext.Add<Review>(model);
        ...
        public IEnumerable<Review> GetAll()
        {
            try
            {
                var obj = _appDbContext.Reviews.ToList();
        ...
        public Review GetById(string? Id)
        {
            try
            {
                if (Id != null)
                {
                    var Obj = _appDbContext.Reviews.FirstOrDefault(x => x.Id == Id);
                    if (Obj != null) return Obj;
                    return null;
                }
                    return null;
